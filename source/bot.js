...
Object.iterate( commands.commands, function ( cmdName, fun ) {
	var cmd = {
		name : cmdName,
		fun  : fun.bind(bot),
		permissions : {
			del : 'NONE',
			use : commands.details.privilegedCommands[ cmdName ] ? 'OWNER' : 'ALL'
		},
		description : commands.details.descriptions[ cmdName ],
		unTellable : commands.details.unTellable[ cmdName ],
		async : commands.commands[ cmdName ].async
	};

	if ( commands.details.communal[cmdName] ) {
